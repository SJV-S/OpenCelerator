...
class FigureManager(QWidget):
    ...
    def phase_line_from_form(self, y, text, date):
        try:
            ...
            if date in self.Chart.date_to_pos.keys():
                ...
                phase_text = self.ax.text(x, y, text, bbox=dict(facecolor='white', edgecolor='black'), ha="left", va="bottom", fontsize=self.phase_font)
    ...
    def phase_line_handle_click(self, event, text):
        if event.inaxes:
            if event.inaxes is not None:  # In case user clicks outside coordinate system
                ...
                self.temp_phase_line_text = self.ax.text(x, y, text, bbox=dict(facecolor='white', edgecolor='black'), ha="left", va="bottom", fontsize=self.phase_font)
                self.refresh()
    ...
    def plot_trend_est(self, x_slice_mean, trend_vals_mean, cel_est, color, adjust=2):
        self.trend_temp_est = self.ax.text(int(x_slice_mean), trend_vals_mean * adjust, cel_est, fontsize=self.trend_est_font, color=color, ha="center", weight="bold")
        self.refresh()
    ...
    def aim_click_info(self, event, note):
        if event.xdata is not None:
            ...
            if self.aim_first_click_x is None:
                ...
                self.aim_temp_note = self.ax.text(xmax, self.aim_y, note, bbox=dict(facecolor='white', edgecolor='black'), ha="left", va="bottom", fontsize=self.aim_font)
