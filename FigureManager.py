...
class FigureManager(QWidget):
    ...
    def setup_layout(self):
        if not hasattr(self, 'canvas'):
            # Only create the canvas if it does not already exist
            self.canvas = FigureCanvas(self.figure)
            self.layout = QVBoxLayout()
            self.layout.addWidget(self.canvas)
            self.setLayout(self.layout)
        else:
            # If the canvas exists, update its figure
            self.canvas.figure = self.figure
            self.canvas.draw_idle()

        # Adjust the fixed size based on the current figure dimensions
    ...
    def plot_corr_points(self, x, y):
        ...
        corr_points = self.ax.plot(x, y, color='black', marker='o', linewidth=0.5, markersize=3, linestyle='')
    ...
    def plot_err_points(self, x, y):
        # Assumes x are integers
        err_points = self.ax.plot(x, y, color='black', marker='x', linewidth=0.5, markersize=5.5, linestyle='')
    ...
    def plot_floor_points(self, x, y):
        # Assumes x are integers
        floor_points = self.ax.plot(x, y, linestyle="", color="black", marker="_", markersize=5, markeredgewidth=1.5)
    ...
    def new_chart(self, start_date):
        ...
        if hasattr(self, 'canvas'):
            # If the canvas exists, just update the figure associated with it
            self.canvas.figure = self.figure
            self.canvas.draw_idle()
        else:
            # If there is no canvas, delegate to setup_layout to create it
            self.setup_layout()
    ...
    def view_update_credit_lines(self, row1, row2, row3):
        ...
        credit = row1 + '\n\n' + row2 + '\n\n' + row3
        self.credit_lines_object = self.ax.text(0, - 0.22, credit, transform=self.Chart.trans, fontsize=8, color=self.Chart.style_color, ha='left', va='center')
    ...
    def settings_color_code_data_points(self, color):
        if color:
            if self.chart_objects['corr_obj']:
                self.chart_objects['corr_obj'].set_color('#007f00')
            if self.chart_objects['err_obj']:
                self.chart_objects['err_obj'].set_color('#cc0000')
        else:
            if self.chart_objects['corr_obj']:
                self.chart_objects['corr_obj'].set_color('black')
            if self.chart_objects['err_obj']:
                self.chart_objects['err_obj'].set_color('black')
