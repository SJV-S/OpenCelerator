...
5) By version 0.7.0, it will be possible to add a miscellaneous data category. Any column starting with “o” will be interpreted as this data type. Unlike the dot and x categories, misc data is not divided by the timing floor (if used). It is not possible to place celeration lines on the misc data points either. However, the same kind of custom data styling options will be available.
