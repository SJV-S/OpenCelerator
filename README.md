...
5) By version 0.7.0, it will be possible to add a miscellaneous data category. Any column starting with “o” will be interpreted as this data type. Unlike the dot and x categories, misc data is not turned into a frequency count using the timing floor if used. Nor can celeration lines be placed on it. However, the same kind of custom data styling options are available. 
