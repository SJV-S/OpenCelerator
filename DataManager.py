...
class DataManager:
    ...
    def __init__(self):
        if not hasattr(self, 'initialized'):
            ...
            self.place_zero_counts_below_floor = True
    ...
    def get_replot_points(self, date_to_x, kind):
        ...
        if kind == 'm':
            ...
            if self.place_zero_counts_below_floor:
                # Place below timing floor
                df.loc[:, 'm_safe'] = df['m'].replace(0, np.nan)  # Replace 0 in 'm' with NaN for safety
                y = np.where(df[kind] == 0, (1 / df['m_safe']) * 0.8, df[kind] / df['m_safe'])
            else:
                # Don't display
