    ...
    def _setup_axes(self):
        ...
        for tick, date, month_width in zip(self.top_x_ticks, self.month_name_labels, self.month_tick_nums[1:]):
            if month_width > 3:
                ...
                self.ax.text(tick + horizontal_off_set, self.top_x_labels_vert_pos, date, transform=self.trans, fontname=self.font, fontsize=self.month_label_size, color=self.style_color, ha="center")
...
class MonthlyTemplate(Chart):
    ...
    def _setup_axes(self):
        ...
        for tick in self.top_x_ticks[:-1]:
            ...
            self.ax.text(tick + 6, self.year_vert_pos, str(year)[-2:], transform=self.trans, fontname=self.font, fontsize=self.year_font_size, color=self.style_color, ha="center")
...
class YearlyTemplate(Chart):
    ...
    def _setup_axes(self):
        ...
        for tick in self.top_x_ticks[:-1]:
            year = self.all_dates[tick].year
            self.ax.text(tick + 5, self.year_vert_pos, str(year), transform=self.trans, fontname=self.font, fontsize=self.year_font_size, color=self.style_color, ha="center")
        ...
        if self.major_grid_on:
            for year in self.bottom_x_ticks[::5]:
                g1 = self.ax.axvline(year, color=self.custom_grid_color, linewidth=self.major_grid_width, visible=True)
                self.major_grid_line_objects.append(g1)
            for power in [0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000, 100000, 1000000]:
                g2 = self.ax.axhline(power, color=self.custom_grid_color, linewidth=self.major_grid_width, visible=True)
                self.major_grid_line_objects.append(g2)
