...
from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QTabWidget, QVBoxLayout, QHBoxLayout, QPushButton, \
    QGroupBox, QRadioButton, QVBoxLayout, QLineEdit, QLabel, QDateEdit, QListWidget, QFileDialog, QCheckBox, QButtonGroup, QDialog
...
- The outer ticks on the chart is not the same width and their corresponding inner lines
- Settings
- - Place zero counts below timing floor, or don't display them at all
- The edit dot in aims need to be cleared when moving to another mode
- What happens if the user have duplicate date entries for import?
- Add button for generating a new default chart (so you don't have to turn of the app every time for that purpose)
- a manual entry currently resets the color and connecting lines for the dots and xs
'''
...
class ChartApp(QMainWindow):
    ...
    def setup_settings_tab(self):
        ...
        settings_color_data_points = QCheckBox('Color data points')
        settings_layout.addWidget(settings_color_data_points)
        settings_color_data_points.stateChanged.connect(self.figure_manager.settings_color_code_data_points)
    ...
    def setup_manual_mode(self):
        ...
        self.radio_dot = QRadioButton("Corrects")
        self.radio_x = QRadioButton("Errors")
    ...
    def setup_view_mode(self):
        # Checkboxes
        self.minor_grid_check = QCheckBox('Minor gridlines')
        self.major_grid_check = QCheckBox('Major gridlines')
        self.dots_check = QCheckBox('Dots')
        self.xs_check = QCheckBox('Xs')
        self.dot_trends_check = QCheckBox('Dot trends')
        self.x_trends_check = QCheckBox('X trends')
        self.timing_floor_check = QCheckBox('Timing floor')
        self.timing_grid_check = QCheckBox('Timing grid')
        self.phase_lines_check = QCheckBox('Phase lines')
        self.aims_check = QCheckBox('Aims')
        self.dot_lines_check = QCheckBox('Dot lines')
        self.x_lines_check = QCheckBox('X lines')
        ...
        for check_box in [self.minor_grid_check,
            ...
            self.home_layout.addWidget(check_box)

        self.timing_grid_check.setChecked(False)
        self.dot_lines_check.setChecked(False)
        self.x_lines_check.setChecked(False)
        self.home_layout.addWidget(self.dot_lines_check)
        self.home_layout.addWidget(self.x_lines_check)
        self.home_layout.addWidget(self.timing_grid_check)
    ...
    def setup_trend_mode(self):
        ...
        self.trend_radio_dot = QRadioButton("Corrects")
        self.trend_radio_x = QRadioButton("Errors")
